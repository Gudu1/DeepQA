...
sys.path.append(os.path.join(os.path.dirname(
    os.path.realpath(__file__)), os.pardir))   # print os.path.dirname(__file__)所在脚本是以完整路径被运行的， 那么将输出该脚本所在的完整路径;所在脚本是以相对路径被运行的， 那么将输出空目录
import time                                    # 而os.path.realpath(__file__),取的是脚本的完整路径  os.pardir取父目录
import shutil
import MySQLdb
    ...
    def process_chat_history_by_ids(self, user1, user2):
        ...
        if dialog_user_number == 2:
            ...
            for every_chat_record in chat_records:
                ...
                +  toUserId + " +++$+++ " + msgId + " +++$+++ " + segmenter.process_sentence(msg) + '\n'
...
if __name__ == "__main__":
    ...
    try:
        ...
        record_file_path = chat_data_path + os.sep + "snap_lines.txt"
        lineids_file_path = chat_data_path + os.sep + "snap_conversation.txt"
